...
{                                                   // no guarantee all given pages are mapped
  ...
  ret_rg->rg_end = addr; //at first rg_end = rg_start
...
{ 
  ...
  for(pgit = 0; pgit < req_pgnum; pgit ++)
  {
    ...
    {
      // fpit->fp_next = malloc(sizeof(struct framephy_struct));
      // fpit = fpit->fp_next;
      // fpit->fpn = fpn;
      if(temp == NULL)
      {
